...
En este paso final prepararemos el sitio para la producción.

*El paso de crear un archivo gemfile lo hicimos al principio asi que no hace falta que lo repitamos*

#### Plugins

Los **plugins** de Jekyll permiten crear contenido personalizad para nuestro sitio. Hay muchos complementos disponibles o incluso puedemos escribir el nuestro si quisieramos.

Hay tres complementos oficiales que son útiles en casi cualquier sitio de Jekyll:

- jekyll-sitemap : crea un archivo de mapa del sitio para ayudar a los motores de búsqueda a indexar contenido.

- jekyll-feed : crea un feed RSS para tus publicaciones.

- jekyll-seo-tag : agrega metaetiquetas para ayudar con el SEO.

Para usarlos primero tenemos que agregarlos a nuestro *Gemfile*.
Podemos añadirlos a un grupo -> jekyll_plugins y se añadirán automaticamente a jekyll:
...
Luego tenemos que añadir estas líneas a nuestro *_config.yml*:
...
plugins:
  ...
  - jekyll-seo-tag
```
y posteriormente instalarlos ejecutando `bundle update`.

**jekyll-sitemap** no necesita ninguna configuración y creará nuestro mapa del sitio en la compilación.
...
Para usar **jekyll-feed** y **jekyll-seo-tag** tenemos que añadir estas etiquetas a *_layouts/default.html*:
...
A veces es posible que deseemos generar algo en producción pero no en desarrollo. Los scripts de análisis son el ejemplo más común.

Para hacer esto podemos configurar el entorno utilizando la variable de entorno **JEKYLL_ENV** al ejecutar un comando. 

Por ejemplo:

`JEKYLL_ENV=production bundle exec jekyll build`

**JEKYLL_ENV** está disponible liquid -> *jekyll.environment*. Por tanto  para generar solo el script de análisis en producción hariamos lo siguiente:
...
El último paso es llevar el sitio a un servidor de producción. La forma más básica de hacer esto es ejecutar una compilación de producción:

`JEKYLL_ENV=production bundle exec jekyll build`

Y posteriormente copiar el contenido de archivo *_site* en nuestro servidor.
